            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
               <div className="bg-white p-10 rounded-3xl shadow-sm border border-slate-100 h-fit">
                  ...
                  <form onSubmit={handleAddSchedule} className="space-y-4">
                     <div>
                        <label className="text-xs font-black uppercase text-slate-400 mb-1 block">Class</label>
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                           ...
                           <input value={schSubject} onChange={(e) => setSchSubject(e.target.value)} className="w-full p-4 border rounded-xl bg-slate-50 font-bold" placeholder="Math" />
                        </div>
                     </div>
                     <div>
                        <label className="text-xs font-black uppercase text-slate-400 mb-1 block">Time Slot</label>
                        <input value={schTime} onChange={(e) => setSchTime(e.target.value)} className="w-full p-4 border rounded-xl bg-slate-50 font-bold" placeholder="07:00 AM - 08:30 AM" />
                     </div>
                     <button type="submit" disabled={loading} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold hover:bg-blue-700 transition">Update Schedule</button>
                  </form>
               </div>
               <div className="bg-white p-10 rounded-3xl shadow-sm border border-slate-100">
                  <h3 className="text-2xl font-bold mb-8 uppercase tracking-tight">Existing Slots</h3>
                  <div className="space-y-3">
                     {schedules.map(s => (
                        <div key={s.id} className="p-4 border border-slate-100 rounded-xl flex justify-between items-center bg-slate-50">
                           <div>
                           ...
                           <button onClick={() => deleteSchedule(s.id)} className="text-red-400 p-2 hover:bg-red-50 rounded-full transition"><i className="fas fa-trash"></i></button>
                        </div>
                     ))}
                     {schedules.length === 0 && <p className="text-slate-400 italic text-center py-12">No schedule entries yet.</p>}
                  </div>
               </div>
            </div>
          } />
